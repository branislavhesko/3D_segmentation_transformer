...
from segmentation_voxel.config.snemi_config import SNEMICOnfig
from segmentation_voxel.config.mode import DataMode
from segmentation_voxel.data.dataset_nucmm import get_data_loaders
from segmentation_voxel.data.snemi_loader import get_data_loaders_snemi
...
class SegmentationTrainer:
    ...
    def __init__(self, config: Config) -> None:
        self.config = config
        self.data_loaders = get_data_loaders_snemi(config)
    ...
    def _visualize(self, model_output):
        vedo.close()
        vol = Volume(model_output[0, :, :, :, :].sigmoid().argmax(0).detach().cpu().numpy())
...
if __name__ == "__main__":
    SegmentationTrainer(SNEMICOnfig()).train()
